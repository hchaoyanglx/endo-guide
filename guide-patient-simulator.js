(function () {
  ...
  function makeCase() {
    ...
    return {
      ...
      interview: { started: false, phase: 'history', setting: scene.id.indexOf('inpatient') >= 0 || scene.id === 'discharge' ? 'inpatient' : 'outpatient', askedHistory: [], selectedExams: [], selectedTests: [], reassessmentExams: [], reassessmentTests: [], impressionDecision: '', finalDecision: '', selectedMeds: [], historyReturns: 0, examReturns: 0, text: { impression: '', diagnosis: '', plan: '' }, submitted: false, score: null },
      activeTab: 'start',
      completed: false,
      seed: newSeed()
    };
  ...
  if (!state.interview) state.interview = { started: false, phase: 'history', setting: 'outpatient', askedHistory: [], selectedExams: [], selectedTests: [], reassessmentExams: [], reassessmentTests: [], impressionDecision: '', finalDecision: '', selectedMeds: [], historyReturns: 0, examReturns: 0, text: { impression: '', diagnosis: '', plan: '' }, submitted: false, score: null };
  if (!state.interview.text) state.interview.text = { impression: '', diagnosis: '', plan: '' };
  ...
  function ctaScore() {
    ...
    var testEarned = criticalTests.reduce(function (n, x) { return n + (it.selectedTests.includes(x.id) ? 2 : 0); }, 0) + optionalTests.reduce(function (n, x) { return n + (it.selectedTests.includes(x.id) ? 1 : 0); }, 0);
    ...
    var max = histMax + examMax + testMax + 2 + 3 + medMax;
    var earned = histEarned + examEarned + testEarned + decisionEarned + textEarned + medEarned;
  ...
  function ctaTranscript() {
    if (!state.interview.askedHistory.length) return '<p class="sim-muted">患者还没有回答任何问题。请从下面选择一个你想主动询问的问题。</p>';
    return state.interview.askedHistory.map(function (id, index) {
  ...
  function ctaReview() {
    ...
    var historyRows = template.history.map(function (item) { var asked = it.askedHistory.includes(item.id); var critical = score.criticalHistory.includes(item); return '<article class="sim-cta-review-row ' + (asked ? 'good' : critical ? 'error' : 'warn') + '"><div>' + status(asked, critical) + '<b>' + esc(ctaGroup(item)) + '</b><p><b>应问：</b>' + esc(item.question) + '</p>' + (asked ? '<p><b>患者回答：</b>' + esc(item.answer) + '</p>' : '<p class="sim-cta-error-text">本次未问到，患者信息不会自动出现。</p>') + '<p><b>为什么：</b>' + esc(item.why || '本例未提供单独解释；应结合相关指南和当前危险程度复核。') + '</p></div></article>'; }).join('');
    var examRows = template.exams.map(function (item) { var selected = it.selectedExams.includes(item.id); var critical = score.criticalExams.includes(item); return '<article class="sim-cta-review-row ' + (selected ? 'good' : critical ? 'error' : 'warn') + '"><div>' + status(selected, critical) + '<b>查体：' + esc(item.label) + '</b><p><b>模拟结果：</b>' + esc(item.result) + '</p><p><b>临床意义：</b>' + esc(item.meaning || '本例未提供单独解释；请回到对应指南路径核对。') + '</p></div></article>'; }).join('');
    var testRows = template.tests.map(function (item) { var selected = it.selectedTests.includes(item.id); var critical = score.criticalTests.includes(item); return '<article class="sim-cta-review-row ' + (selected ? 'good' : critical ? 'error' : 'warn') + '"><div>' + status(selected, critical) + '<b>检查：' + esc(item.name) + '</b><p><b>结果：</b>' + esc(item.result) + '</p><p><b>如何解释：</b>' + esc(item.interpretation) + '</p><p><b>为什么选择：</b>' + esc(item.why || '本例未提供单独解释；请回到指标追查路径核对。') + '</p>' + (item.workupId ? '<button class="sim-link" type="button" data-sim-link="workup:' + esc(item.workupId) + '">打开对应指标追查</button>' : '') + '</div></article>'; }).join('');
    ...
    return '<div class="sim-review sim-cta-review"><div class="sim-review-head"><span class="sim-kicker">CTA 站后解析</span><h3>' + (score.ratio >= 80 ? '主线覆盖较完整' : '请按红色缺口重新训练') + '</h3><p>本次为' + esc(ctaSettingLabel()) + '；解析在提交后才开放，练习过程没有提前显示答案。</p><div class="sim-score"><b>' + score.ratio + '</b><span>/ 100 学习反馈分</span></div></div><div class="sim-score-grid"><div><b>问诊</b><span>' + score.histEarned + '/' + score.histMax + '</span></div><div><b>查体</b><span>' + score.examEarned + '/' + score.examMax + '</span></div><div><b>检查</b><span>' + score.testEarned + '/' + score.testMax + '</span></div><div><b>判断</b><span>' + score.decisionEarned + '/2</span></div><div><b>文字总结</b><span>' + score.textEarned + '/3</span></div><div><b>药物安全</b><span>' + (score.medMax ? score.medEarned + '/' + score.medMax : '未设候选') + '</span></div></div><div class="sim-review-block sim-cta-guide"><h4>按 CTA 顺序回放</h4><p>资料收集（主动问诊）→ 初步印象（诊断/鉴别及理由）→ 针对性查体 → 辅助检查与诊断 → 最终判断与治疗 → 治疗后复评（住院场景）。</p><p class="sim-muted">红色=关键漏问或选择了药物卡标注的停用/避免方向；黄色=可补充项目；绿色=本次已覆盖。每条“为什么”均来自本例已标注的指南路径字段。</p></div><div class="sim-review-block"><h4>一、问诊逐项解析</h4><div class="sim-cta-review-list">' + historyRows + '</div></div><div class="sim-review-block"><h4>二、查体逐项解析</h4><div class="sim-cta-review-list">' + examRows + '</div></div><div class="sim-review-block"><h4>三、辅助检查逐项解析</h4><div class="sim-cta-review-list">' + testRows + '</div></div><div class="sim-review-block"><h4>四、诊断/鉴别诊断与治疗决策解析</h4><div class="sim-cta-review-list">' + decisionRows + '</div>' + (medRows ? '<div class="sim-cta-review-list">' + medRows + '</div>' : '<p class="sim-muted">本例没有挂接药物候选卡；不要自行补写剂量，回到药物剂量卡按适应证和禁忌核对。</p>') + '</div><div class="sim-review-block"><h4>本例指南依据</h4><ul>' + template.source.map(function (x) { return '<li>' + esc(x) + '</li>'; }).join('') + '</ul></div><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-retry">保留本例，重新做一遍</button><button class="sim-secondary" type="button" data-sim-action="new">抽取新患者</button></div></div>';
  }
  function renderCta() {
    var it = state.interview;
    if (it.submitted) return ctaReview();
    if (!it.started) { return '<div class="sim-cta-welcome"><div class="sim-section-title"><div><span class="sim-kicker">CTA 一问一答训练站</span><h3>先问，再获得患者回答</h3><p class="sim-muted">本模式参考你提供的 CTA 病例流程：资料收集 → 初步印象 → 针对性查体 → 辅助检查/诊断 → 治疗与复评。练习时不会显示“为什么”、红旗或正确选项。</p></div></div><div class="sim-patient-card"><span class="sim-pill">本次虚构患者</span><h3>' + esc(state.person.age + ' 岁 · ' + state.person.sex + ' · ' + state.person.job) + '</h3><p><b>主诉：</b>' + esc(state.person.complaint) + '</p><p class="sim-quote">“' + esc(mood.opening) + '”</p><p class="sim-muted">你可以先选择门诊或住院场景；场景只影响流程提示，不会替你生成答案。</p><label class="sim-cta-select"><b>训练场景</b><select data-cta-setting><option value="outpatient"' + (it.setting === 'outpatient' ? ' selected' : '') + '>门诊：首诊/复诊</option><option value="inpatient"' + (it.setting === 'inpatient' ? ' selected' : '') + '>住院：入院/病程/复评</option></select></label></div><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-start">开始资料收集</button><button class="sim-secondary" type="button" data-sim-action="new">换一位患者</button></div></div>'; }
    var phase = it.phase;
    var phaseTitle = ({ history: '第一站：资料收集（主动问诊）', impression: '第二站：初步印象与鉴别', exams: '第三站：针对性体格检查', workup: '第四站：辅助检查', diagnosis: '第五站：最终诊断与下一步', plan: '第六站：治疗方案', reassessment: '第七站：治疗后复评' })[phase] || 'CTA 训练';
    var phaseNote = ({ history: '患者只回答你点选的一个问题；不要把未问到的资料当成已知。', impression: '从当前已获得的信息提出初步印象和鉴别；本阶段可返回补问。', exams: '只选择能回答当前问题的查体；选择后才显示模拟结果。', workup: '选择能确认诊断、排除危险或改变下一步的检查；结果不附带解析。', diagnosis: '选择你的最终判断并写下支持/不支持理由；解析会在提交后开放。', plan: '选择药物或治疗方向并写安全监测；药物卡的作用与禁忌暂不显示。', reassessment: '住院场景：治疗后重新选择需要复查的查体/检查，再决定是否继续调整。' })[phase];
    var body = '<div class="sim-section-title"><div><span class="sim-kicker">' + esc(ctaSettingLabel()) + '</span><h3>' + esc(phaseTitle) + '</h3><p class="sim-muted">' + esc(phaseNote) + '</p></div><span class="sim-stage">已问 ' + it.askedHistory.length + '/' + template.history.length + ' 项</span></div>';
    if (phase === 'history') {
      var questions = ctaQuestionOptions();
      body += '<div class="sim-cta-transcript">' + ctaTranscript() + '</div><h4>选择你下一句要问的问题</h4><div class="sim-cta-question-list">' + (questions.length ? questions.map(function (item) { return ctaChoiceButton('cta-question', item.id, item.question, false, ctaGroup(item)); }).join('') : '<p class="sim-muted">本例预设问题已全部问过，可以进入初步印象。</p>') + '</div><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-impression">结束问诊，进入初步印象</button></div>';
    } else if (phase === 'impression') {
      body += '<div class="sim-cta-transcript"><h4>目前患者已回答</h4>' + ctaTranscript() + '</div><h4>选择你的初步判断（不显示对错）</h4><div class="sim-choice-list">' + template.decisions.map(function (item) { return ctaChoiceButton('cta-impression-choice', item.id, item.label, item.id === it.impressionDecision, '初步诊断/鉴别选项'); }).join('') + '</div><label class="sim-note-field"><b>初步印象与鉴别理由</b><small>写支持证据、尚不能排除的危险情况和下一步需要验证的内容。</small><textarea data-cta-text="impression" placeholder="先独立书写，不要等解析...">' + esc(it.text.impression || '') + '</textarea></label><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-exams">进入针对性查体</button><button class="sim-secondary" type="button" data-sim-action="cta-history-return">返回补问病史</button></div>';
    } else if (phase === 'exams') {
      body += '<div class="sim-cta-transcript"><h4>问诊记录</h4>' + ctaTranscript() + '</div><h4>选择要做的查体</h4><div class="sim-choice-list">' + template.exams.map(function (item) { return ctaChoiceButton('cta-exam', item.id, item.label, it.selectedExams.includes(item.id), it.selectedExams.includes(item.id) ? '已获得模拟结果' : '选择后显示结果'); }).join('') + '</div><div class="sim-cta-results">' + template.exams.filter(function (item) { return it.selectedExams.includes(item.id); }).map(function (item) { return '<article class="sim-detail"><h4>' + esc(item.label) + '</h4><p><b>模拟结果：</b>' + esc(item.result) + '</p></article>'; }).join('') + '</div><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-workup">进入辅助检查</button><button class="sim-secondary" type="button" data-sim-action="cta-history-return">返回补问病史（仅按 CTA 规则保留一次）</button></div>';
    } else if (phase === 'workup') {
      body += '<h4>选择辅助检查</h4><div class="sim-choice-list">' + template.tests.map(function (item) { return ctaChoiceButton('cta-test', item.id, item.name, it.selectedTests.includes(item.id), it.selectedTests.includes(item.id) ? '已获得模拟结果' : item.stage || '选择后显示结果'); }).join('') + '</div><div class="sim-cta-results">' + template.tests.filter(function (item) { return it.selectedTests.includes(item.id); }).map(function (item) { return '<article class="sim-detail"><h4>' + esc(item.name) + '</h4><p><b>模拟结果：</b>' + esc(item.result) + '</p></article>'; }).join('') + '</div><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-diagnosis">进入最终诊断</button><button class="sim-secondary" type="button" data-sim-action="cta-exams-return">返回补充查体（最多两次）</button></div>';
    } else if (phase === 'diagnosis') {
      body += '<h4>选择最终诊断/下一步判断</h4><div class="sim-choice-list">' + template.decisions.map(function (item) { return ctaChoiceButton('cta-final-choice', item.id, item.label, item.id === it.finalDecision, '最终判断选项'); }).join('') + '</div><label class="sim-note-field"><b>最终诊断与鉴别诊断理由</b><small>写出支持证据、反对证据、仍需排除的危险情况和选择本方案的原因。</small><textarea data-cta-text="diagnosis" placeholder="先独立书写，不要等解析...">' + esc(it.text.diagnosis || '') + '</textarea></label><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-plan">进入治疗方案</button><button class="sim-secondary" type="button" data-sim-action="cta-exams-return">返回补充检查（最多两次）</button></div>';
    } else if (phase === 'plan') {
      var refs = ctaMedicationOptions();
      body += '<h4>选择治疗/药物方向（不显示作用与禁忌）</h4><div class="sim-choice-list">' + (refs.length ? refs.map(function (ref) { return ctaChoiceButton('cta-med', ref.name, ref.name, it.selectedMeds.includes(ref.name), '选择后只记录你的方案'); }).join('') : '<p class="sim-muted">本例未挂接药物候选卡；请写治疗原则，提交后回到指南和药物卡复核。</p>') + '</div><label class="sim-note-field"><b>治疗计划与监测/宣教</b><small>写治疗目标、监测项目、复评节点、患者教育和升级条件；不要凭空补剂量。</small><textarea data-cta-text="plan" placeholder="先独立书写，不要等解析...">' + esc(it.text.plan || '') + '</textarea><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-submit">提交并查看解析</button>' + (it.setting === 'inpatient' ? '<button class="sim-secondary" type="button" data-sim-action="cta-reassessment">进入治疗后复评</button>' : '') + '</div>';
    } else if (phase === 'reassessment') {
      body += '<div class="sim-notice"><b>住院病程节点：</b>治疗后病情会变化；请再次选择需要复查的查体和检查，然后决定是否继续调整。</div><h4>治疗后需要复查的查体</h4><div class="sim-choice-list">' + template.exams.map(function (item) { return ctaChoiceButton('cta-reexam', item.id, item.label, it.reassessmentExams && it.reassessmentExams.includes(item.id), '治疗后复评'); }).join('') + '</div><h4>治疗后需要复查的检查</h4><div class="sim-choice-list">' + template.tests.map(function (item) { return ctaChoiceButton('cta-retest', item.id, item.name, it.reassessmentTests && it.reassessmentTests.includes(item.id), '治疗后复评'); }).join('') + '</div><div class="sim-actions"><button class="primary" type="button" data-sim-action="cta-submit">提交并查看解析</button></div>';
    }
    return body;
  }
  ...
  function renderBase() {
    ...
    var tabs = [{ id: 'start', label: '患者与沟通' }, { id: 'interview', label: 'CTA一问一答' }, { id: 'history', label: '问诊' }, { id: 'exams', label: '床旁查体' }, { id: 'tests', label: '选择检查' }, { id: 'decisions', label: '下一步判断' }, { id: 'review', label: '复盘' }];
  ...
  function handle(action, id) {
    ...
    if (action === 'cta-start') { state.interview.started = true; state.interview.phase = 'history'; state.interview.submitted = false; state.activeTab = 'interview'; saveState(); render(); return; }
    ...
    if (action === 'cta-impression') { state.interview.phase = 'impression'; saveState(); render(); return; }
    if (action === 'cta-history-return') {
      if (state.interview.phase === 'impression') { state.interview.historyReturns += 1; state.interview.phase = 'history'; }
      else if (state.interview.phase === 'exams' && state.interview.examReturns < 1) { state.interview.examReturns += 1; state.interview.phase = 'history'; }
      saveState(); render(); return;
    }
    if (action === 'cta-exams') { state.interview.phase = 'exams'; saveState(); render(); return; }
    if (action === 'cta-exam') {
      var examId = id;
    ...
    if (action === 'cta-workup') { state.interview.phase = 'workup'; saveState(); render(); return; }
    if (action === 'cta-test') {
      var testId = id;
      if (state.interview.selectedTests.includes(testId)) state.interview.selectedTests = state.interview.selectedTests.filter(function (x) { return x !== testId; });
      else state.interview.selectedTests.push(testId);
      saveState(); render(); return;
    }
    ...
    if (action === 'cta-diagnosis') { state.interview.phase = 'diagnosis'; saveState(); render(); return; }
    ...
    if (action === 'cta-plan') { state.interview.phase = 'plan'; saveState(); render(); return; }
    if (action === 'cta-med') {
    ...
    if (action === 'cta-submit') { state.interview.submitted = true; state.interview.score = ctaScore().ratio; saveState(); render(); return; }
    if (action === 'cta-retry') {
      var currentSetting = state.interview.setting;
      state.interview = { started: true, phase: 'history', setting: currentSetting, askedHistory: [], selectedExams: [], selectedTests: [], reassessmentExams: [], reassessmentTests: [], impressionDecision: '', finalDecision: '', selectedMeds: [], historyReturns: 0, examReturns: 0, text: { impression: '', diagnosis: '', plan: '' }, submitted: false, score: null };
