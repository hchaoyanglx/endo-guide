(function () {
  ...
  function renderCta() {
    ...
    var phaseNote = ({ history: '患者只回答你点选的一个问题；不要把未问到的资料当成已知。', impression: '从当前已获得的信息提出初步印象和鉴别；完成后单向进入查体。', exams: '只选择能回答当前问题的查体；选择后才显示模拟结果。', workup: '选择能确认诊断、排除危险或改变下一步的检查；结果不附带解析。', diagnosis: '选择你的最终判断并写下支持/不支持理由；解析会在提交后开放。', plan: '选择药物或治疗方向并写安全监测；药物卡的作用与禁忌暂不显示。', reassessment: '住院场景：治疗后重新选择需要复查的查体/检查，再决定是否继续调整。' })[phase];
